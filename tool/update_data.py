...
df_counts = df_counts.dropna(subset=['年月日', '陰性人数', '陽性人数', '一般相談件数', '受診・相談センター相談件数', '退院者数', '死亡者数'])

# 欠損値を0埋め
df_counts = df_counts.fillna(0)

# データ形式の指定
df_counts = df_counts.astype({'年月日': str, 'PCR検査数': int, '抗原検査数': int, '陰性人数': int, '陽性人数': int, '一般相談件数': int, '受診・相談センター相談件数': int, '退院者数': int, '死亡者数': int})
...
# 受診・相談センター相談件数
df_querents = df_counts.loc[:, ("年月日", "受診・相談センター相談件数")].copy()
df_querents.rename(columns={"年月日": "日付", "受診・相談センター相談件数": "小計"}, inplace=True)
